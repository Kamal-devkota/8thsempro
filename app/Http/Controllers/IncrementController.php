...
class IncrementController extends Controller
{
    public function newIncrementSearch()
    {
        $employees = User::whereBetween('access_label', [2, 3])
            ...
            ->get()
            ->toArray();
        return view('admin.hrm.increment.newIncrementSearch', compact('employees'));
    }
    ...
    public function incrementList()
    {
        ...
        return view('admin.hrm.increment.incrementList', compact('increments'));
    }
    public function salaryStatementSearch()
    {
        $employees = SalaryPayment::all();
        return view('admin.hrm.increment.salaryStatementSearch', compact('employees'));
    }
    public function salaryStatementView(Request $request)
    {
        ...
        if ($request->daterange == '' or $empid == '') {
            ...
            return view('admin.hrm.increment.salaryStatementView', compact('salarysheets', 'startdate', 'enddate', 'datetotal', 'empid'));
        }
    }
    public function salaryStatementPreview(Request $request)
    {
        ...
        if ($empid == '' or $startdate = '' or $enddate = '') {
            ...
            $datetotal = DB::table('salary_payments')->whereBetween('payment_month', [$request->date1, $request->date2])
                ->where('user_id', $empid)->sum('gross_salary');
            return view('admin.hrm.increment.salaryStatementPreview', compact('salarystats', 'startdate', 'enddate', 'datetotal', 'empid'));
        }
    }
    public function salaryStatementPdf(Request $request)
    {
        ...
        if ($empid == '' or $startdate = '' or $enddate = '') {
            ...
            $pdf = PDF::loadView('admin.hrm.increment.salaryStatementPdf', compact('salarystats', 'startdate', 'enddate', 'datetotal', 'empid'));
